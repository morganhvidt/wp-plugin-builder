  ...
  try {
    await Promise.all(
      plugins.map(async (plugin) => {
        ...
        const productionFolder = pluginConfig.productionFolder || `production/plugins/${pluginConfig.slug}`;
        ...
        if (arg === 'production') {
          ...
          await zipFolder(productionFolder, `${zipPath}/${pluginConfig.slug}-${pluginConfig.version}.zip`);
        }
      }),
    );
...
function watchPlugin(plugin) {
  ...
  const productionFolder = plugin.productionFolder || `production/${plugin.slug}`;

  watcher = fs.watch(sourceFolder, { recursive: true }, (eventType, filename) => {
    console.log(`📁 File ${filename} changed (${eventType})`);

    // Check if the changed file matches any of the exclude patterns
    if (defaultExcludePatterns.some((pattern) => matchPattern(filename, pattern))) {
    ...
    timeoutIds[plugin.slug] = setTimeout(async () => {
      try {
        ...
        await copyFilesAndFolders(sourceFolder, productionFolder, plugin.excludePatterns, replacements(plugin), plugin);
      } catch (err) {
        console.error(`🤬 Error updating ${filename} in ${plugin.slug}:`, err);
      }
 ...
 */
async function copyFilesAndFolders(src, dist, excludePatterns = [], replacements = [], pluginConfig) {
  ...
  await Promise.all(
    files.map(async (file) => {
      ...
      if (stat.isDirectory()) {
        ...
        await copyFilesAndFolders(srcPath, distPath, excludePatterns, replacements, pluginConfig);
      } else {
        ...
        if (replacements.length > 0) {
          ...
          if (pluginConfig.type === 'free') {
            const blockRegex = /(\/\/ @if type = 'premium'\s[\s\S]*?\/\/ @endif\s)/g;
            fileContent = fileContent.replace(blockRegex, '');

            const commentRegex = /(\/\/ @if type = 'free'\s)([\s\S]*?)(\/\/ @endif\s)/g;
            fileContent = fileContent.replace(commentRegex, '$2');
          } else if (pluginConfig.type === 'premium') {
            const blockRegex = /(\/\/ @if type = 'free'\s[\s\S]*?\/\/ @endif\s)/g;
            fileContent = fileContent.replace(blockRegex, '');

            const commentRegex = /(\/\/ @if type = 'premium'\s)([\s\S]*?)(\/\/ @endif\s)/g;
            fileContent = fileContent.replace(commentRegex, '$2');
          }
...
async function minifyFiles(currentPlugin) {
  ...
  const promises = currentPlugin.minify.map(async (config) => {
    ...
    try {
      ...
      console.log(`- ${path.basename(inputPath)} minified ${Math.round(efficiency)}%`);
    } catch (error) {
